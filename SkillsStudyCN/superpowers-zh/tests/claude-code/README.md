...
此测试套件验证技能是否正确加载以及 Claude 是否按预期遵循它们。测试以无头模式 (`claude -p`) 调用 Claude Code 并验证行为。

## 要求

- 已安装 Claude Code CLI 并添加到 PATH（`claude --version` 应可正常工作）
- 已安装本地 superpowers 插件（请参阅主 README 了解安装方法）
...
# 验证响应
assert_contains "$output" "expected behavior" "技能描述行为"
...
使用 `--verbose` 时，你将看到完整的 Claude 输出：
```bash
