...
使用原生 OpenCode 插件架构为 OpenCode.ai 添加完整的 superpowers 支持，该架构与现有的 Codex 实现共享核心功能。

## 背景

OpenCode.ai 是一个类似于 Claude Code 和 Codex 的编码代理。之前尝试将 superpowers 移植到 OpenCode（PR #93，PR #116）使用的是文件复制方法。本设计采用不同的方法：使用 OpenCode 的 JavaScript/TypeScript 插件系统构建原生 OpenCode 插件，同时与 Codex 实现共享代码。
...
- **调用方式**：由主代理自动派发或使用 `@mention` 语法手动调用
...
3. **注入工具映射说明**
   ...
   **OpenCode 的工具映射：**
   ...
   - `Task` 与子代理 → 使用 OpenCode 子代理系统（@mention）
