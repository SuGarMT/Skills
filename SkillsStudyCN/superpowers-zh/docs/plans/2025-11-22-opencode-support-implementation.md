...
**目标：** 使用原生 JavaScript 插件为 OpenCode.ai 添加完整的 superpowers 支持，该插件与现有的 Codex 实现共享核心功能。

**架构：** 将通用技能发现/解析逻辑提取到 `lib/skills-core.js`，重构 Codex 以使用它，然后使用 OpenCode 的原生插件 API 构建 OpenCode 插件，包含自定义工具和会话钩子。
 ...
 * @returns {boolean} - 如果有更新可用则返回 true
 */
...
在文件顶部现有 require 语句之后（约第 6 行），添加：
...
```javascript
    'session.started': async () => {
      ...
      const toolMapping = `
**OpenCode 的工具映射：**
当技能引用您没有的工具时，请替换为 OpenCode 等效工具：
- \`TodoWrite\` → \`update_plan\`（您的计划/任务跟踪工具）
- \`Task\` 工具与子代理 → 使用 OpenCode 的子代理系统（@mention 语法或自动派发）
- \`Skill\` 工具 → \`use_skill\` 自定义工具（已可用）
- \`Read\`、\`Write\`、\`Edit\`、\`Bash\` → 使用您的原生工具
...
`;
      ...
      const updateNotice = hasUpdates ?
        '\n\n⚠️ **有更新可用！** 运行 `cd ~/.config/opencode/superpowers && git pull` 来更新 superpowers。' :
        '';
...
**以下是您的 'superpowers:using-superpowers' 技能的完整内容 - 这是您使用技能的入门指南。对于所有其他技能，请使用 'use_skill' 工具：**
...
- `Task` 与子代理 → 使用 `@mention` 语法调用 OpenCode 子代理
...
运行：`tree -L 2 .opencode/`（如果没有 tree 则使用 `find .opencode -type f`）
预期：
```
.opencode/
├── INSTALL.md
...
- [ ] 创建了 `lib/skills-core.js`，包含所有核心函数
- [ ] `.codex/superpowers-codex` 已重构为使用共享核心
- [ ] Codex 命令仍然正常工作（find-skills、use-skill、bootstrap）
- [ ] 创建了带有工具和钩子的 `.opencode/plugin/superpowers.js`
- [ ] 创建了安装指南
- [ ] 更新了 README 和 RELEASE-NOTES
- [ ] 所有更改已提交
- [ ] 工作树干净
