...
技能由 OpenCode 的原生技能系统发现。每个技能都有一个带 YAML 前置元数据的 `SKILL.md` 文件。
...
3. 检查技能结构：每个技能需要一个带有效前置元数据的 `SKILL.md` 文件

### Windows：模块未找到错误

如果你看到 `Cannot find module` 错误：
- **原因：** Git Bash 的 `ln -sf` 复制文件而不是创建符号链接
- **解决方案：** 使用 `mklink /J` 目录联接（参见 Windows 安装步骤）
