...
本文档描述如何测试 Superpowers 技能，特别是像 `subagent-driven-development` 这样复杂技能的集成测试。

## 概述

测试涉及子代理、工作流和复杂交互的技能需要在无头模式下运行实际的 Claude Code 会话，并通过会话记录验证其行为。
...
3. **验证**：解析会话记录（`.jsonl` 文件）以验证：
   ...
   - 测试通过
   - Git 提交显示正确的工作流程
4. **Token 分析**：按子代理显示 token 使用情况细分
...
- **主会话使用量**：协调器（你或主 Claude 实例）的 token 使用量
- **每个子代理细分**：每次 Task 调用包含：
  - Agent ID
  - 描述（从提示中提取）
  - 消息数量
  - 输入/输出 token
  - 缓存使用量
  - 估算成本
- **总计**：整体 token 使用量和成本估算

### 理解输出

- **高缓存读取**：好现象 - 表示提示缓存正在工作
- **主会话高输入 token**：正常 - 协调器拥有完整上下文
- **每个子代理成本相似**：正常 - 每个任务复杂度相似
...
2. **解析记录**：不要 grep 用户可见输出 - 解析 `.jsonl` 会话文件
3. **授予权限**：使用 `--permission-mode bypassPermissions` 和 `--add-dir`
4. **从插件目录运行**：技能只有在从 superpowers 目录运行时才能加载
5. **显示 token 使用量**：始终包含 token 分析以了解成本
6. **测试真实行为**：验证实际创建的文件、通过的测试、提交的 commit
